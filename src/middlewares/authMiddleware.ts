...
const verifyAdmin = async (req: Request, _res: Response, next: NextFunction): Promise<void> => {
  try {
    if (req.jwtDecoded?.role !== ROLES.ADMIN) {
      throw new ApiError(StatusCodes.FORBIDDEN, 'Bạn cần quyền quản trị viên để thực hiện chức năng này')
    }
    next()
  } catch (error) {
    next(error)
...
const requirePermission = (permissionName: string) => {
  return async (req: Request, _res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const hasPermission = await permissionModel.checkUserPermission(parseInt(userId, 10), permissionName)

      if (!hasPermission) {
        throw new ApiError(StatusCodes.FORBIDDEN, `Bạn không có quyền "${permissionName}" để thực hiện hành động này`)
      }
...
const requireAnyPermission = (permissionNames: string[]) => {
  return async (req: Request, _res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const hasAnyPermission = await permissionModel.checkUserAnyPermission(parseInt(userId, 10), permissionNames)

      if (!hasAnyPermission) {
        throw new ApiError(StatusCodes.FORBIDDEN, `Bạn cần có ít nhất 1 trong các quyền: ${permissionNames.join(', ')}`)
      }
...
const requireAllPermissions = (permissionNames: string[]) => {
  return async (req: Request, _res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const hasAllPermissions = await permissionModel.checkUserAllPermissions(parseInt(userId, 10), permissionNames)

      if (!hasAllPermissions) {
        throw new ApiError(StatusCodes.FORBIDDEN, `Bạn cần có tất cả các quyền sau: ${permissionNames.join(', ')}`)
      }
